...
public class Main {
	...
	private static boolean checkWinner(int[][] board, int row, int col) {
		...
		C : for(int[] d : DIRECTIONS) {
			...
			if(prevRow >= 0 && prevRow < N && prevCol >= 0 && board[prevRow][prevCol] == pos) continue;
			
			for(int offset = 1; offset <= WINNING_COUNT; offset++) {
				int nextRow = row + d[0] * offset, nextCol = col + d[1] * offset;
				
				if(nextRow < 0 || nextRow >= N || nextCol >= N ||
					(offset < WINNING_COUNT && board[nextRow][nextCol] != pos) ||
					(offset == WINNING_COUNT && board[nextRow][nextCol] == pos)) continue C;
			}
